...
module UPS
  ...
  class TimeInTransit
    ...
    def request(options)
      ...
      @url = options[:mode] ? TEST_URL + '/TimeInTransit' : PRODUCTION_URL + '/TimeInTransit'
    ...
    def calculate_pickup_date
      ...
      if(in_friday_after_cutoff)
        pickup_date = now.to_datetime + 3
        
      elsif(day_of_week == 6)
        pickup_date = now.to_datetime + 2
     
      # if we're in another weekday but after the cutoff time, our ship date
      # moves to tomorrow
      elsif(now.hour > @order_cutoff_time or day_of_week == 0)
        pickup_date = now.to_datetime + 1
      else
        pickup_date = now
      end
    end
    ...
    def response_to_map(response) 
      ...
      response_code = response_doc.elements.each('//ServiceSummary') do |service_element|
        ...
        if(service_code == "GND")
          ...
          business_transit_days = service_element.elements['EstimatedArrival/BusinessTransitDays'].text
